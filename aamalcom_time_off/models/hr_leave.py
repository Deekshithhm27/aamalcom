...
class HrLeave(models.Model):
    ...
    state = fields.Selection(selection_add=[
        ('hr_specialist', 'HR Specialist Review'),
    ...
    replacement_id = fields.Many2one('hr.employee', string='Replacement Employee')
    is_saudi = fields.Boolean(string='Is Saudi', compute='_compute_is_saudi')
    exit_reentry = fields.Boolean(string='Exit Reentry')
    ere_from_company = fields.Selection([
        ('company', 'From Company'),
        ('self', 'Self')
    ], string='ERE Source')
    ticket = fields.Boolean(string='Ticket')
    ...
    sick_leave_report = fields.Binary(string='Sick Leave Report')
    ...
    def action_confirm(self):
        ...
        for leave in self:
            if leave.leave_type_name == 'Unpaid':
    ...
    def action_refuse(self):
        """Require refuse reason before rejecting."""
        for leave in self:
            if not leave.refuse_reason:
                raise ValidationError(_("Please provide a reason for refusal."))
    ...
    def _check_custom_validations(self):
        for leave in self:
            ...
            if leave_type_name == 'Annual Vacation':
                notice_days = 14 if leave.is_saudi else (60 if leave.exit_reentry and leave.ere_from_company == 'company' and leave.ticket and leave.ticket_from == 'company' else 14)
    ...
    # ------------------------------
    # Refuse — require reason & handle hr_specialist/gm states
    # ------------------------------
    def action_refuse(self):
        """
        Require refuse_reason and preserve original behaviour.
        Accept refusal in intermediate states as well.
        """
        # require reason
        for holiday in self:
            if not holiday.refuse_reason:
                raise UserError(_('Please provide a reason for refusal.'))

        # allow refusal similar to standard, but accept hr_specialist and gm_approve as valid pre-states
        if any(holiday.state not in ['draft', 'confirm', 'validate', 'validate1', 'hr_specialist', 'gm_approve'] for holiday in self):
            raise UserError(_('Time off request must be confirmed or validated in order to refuse it.'))

        # re-use standard refusal behaviour (this will set state and send notifications)
        return super(HrLeave, self).action_refuse()
