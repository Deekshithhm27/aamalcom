...
class SwappingBorderToIqama(models.Model):
    _inherit = 'swapping.border.to.iqama'

    def _schedule_ticket_activity_sbti(self, user_id, summary, note):
        self.activity_schedule(
            act_type_xmlid='aamalcom_ticket_activity.mail_activity_type_ticket_sbti_action',
            date_deadline=None,
            summary=summary,
            note=note,
            user_id=user_id
    ...
    def action_submit_to_review(self):
        result = super(SwappingBorderToIqama, self).action_submit_to_review()
    ...
    def action_done(self):
        result = super(SwappingBorderToIqama, self).action_done()
        activity_type_ticket = self.env.ref(
            'aamalcom_ticket_activity.mail_activity_type_ticket_sbti_action',
            raise_if_not_found=False
        )

        if activity_type_ticket:
            activities = self.env['mail.activity'].sudo().search([
                ('res_model', '=', 'swapping.border.to.iqama'),
                ('res_id', '=', self.id),
