...
class ServiceEnquiry(models.Model):
    ...
    service_request_activity = fields.Char('Service Request Activity')
    document_attachment = fields.Many2one('ir.attachment', 'Document Attachment')
    ...
    state = fields.Selection([
        ...
        ('done', 'Completed'), ('refuse', 'Refuse'), ('cancel', 'Cancel')], string='State', default="draft", copy=False,
        tracking=True)
    service_request_type = fields.Selection([('lt_request', 'Local Transfer'), ('ev_request', 'Employment Visa')],
                                            string="Service Request Type", tracking=True, copy=False)
    service_request_config_id = fields.Many2one('service.request.config', string="Service Request",
                                                domain="[('service_request_type','=',service_request_type)]",
                                                copy=False)
    ...
    service_request = fields.Selection([('new_ev', 'Issuance of New EV'),
                                        ...
                                        ('qiwa', 'Qiwa Contract'), ('gosi', 'GOSI Update'),
                                        ...
                                        (
                                            'emp_secondment_or_cub_contra_ltr',
                                            'Employee secondment / Subcontract letter'),
                                        ('car_loan', 'Car Loan Letter'), ('bank_loan', 'Bank Loan Letter'),
                                        ...
                                        ('soa', 'Statement of account till date'), ('general_query', 'General Query')],
                                       string="Service Requests", related="service_request_config_id.service_request",
                                       store=True, copy=False)

    employee_id = fields.Many2one('hr.employee',
                                  domain="[('custom_employee_type', '=', 'external'),('client_id','=',user_id)]",
                                  string="Employee", store=True, tracking=True, required=True, copy=False)
    iqama_no = fields.Char(string="Iqama No")
    identification_id = fields.Char(string='Border No.')

    transfer_type = fields.Selection(
        [('to_aamalcom', 'To Aamalcom'), ('to_another_establishment', 'To another Establishment')],
    ...
    visa_religion = fields.Selection([('muslim', 'Muslim'), ('non_muslim', 'Non-Muslim'), ('others', 'Others')],
                                     string="Visa Religion")
    ...
    employment_duration = fields.Many2one('employment.duration', string="Duration", tracking=True,
                                          domain="[('service_request_type','=',service_request_type),('service_request_config_id','=',service_request_config_id)]")
    ...
    current_insurance_class = fields.Selection(
        [('class_vip+', 'VIP+'), ('class_vip', 'VIP'), ('class_a', 'A'), ('class_b', 'B'), ('class_c', 'C'),
         ('class_e', 'E')], string="Current Insurance Class")
    class_to_be_changed = fields.Selection(
        [('class_vip+', 'VIP+'), ('class_vip', 'VIP'), ('class_a', 'A'), ('class_b', 'B'), ('class_c', 'C'),
         ('class_e', 'E')], string="Class to be changed to!")
    duration_limit = fields.Selection(
        [('30', '30'), ('60', '60'), ('90', '90'), ('120', '120'), ('150', '150'), ('180', '180'), ('210', '210'),
         ('240', '240'), ('270', '270'), ('300', '300'), ('330', '330'), ('360', '360')], string="Duration")
    ...
    visa_gender = fields.Selection([('male', 'Male'), ('female', 'Female'), ('others', 'Others')],
                                   string="Visa Gender *")
    ...
    attested_from_saudi_cultural = fields.Selection([('yes', 'Yes'), ('no', 'No')],
                                                    string="Degree attested from saudi cultural", copy=False)
    ...
    dependent_document_ids = fields.One2many('dependent.documents', 'ev_enq_dependent_document_id',
    ...
    current_department_ids = fields.Many2many('hr.department', 'service_enquiry_dept_ids', string="Department",
                                              compute="update_departments")
    ...
    # @api.onchange('upload_hr_card')
    # def _check_hr_card_ref(self):
    #     if self.upload_hr_card and not self.hr_card_ref:
    #         raise ValidationError("Ref No.* is required when uploading HR Card Document.")
    #
    # @api.onchange('upload_jawazat_doc')
    # def _check_jawazat_doc_ref(self):
    #     if self.upload_jawazat_doc and not self.jawazat_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Jawazat Document.")
    #
    # @api.onchange('upload_sponsorship_doc')
    # def _check_sponsorship_doc_ref(self):
    #     if self.upload_sponsorship_doc and not self.sponsorship_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Confirmation of Sponsorship.")
    #
    # @api.onchange('upload_payment_doc')
    # def _check_payment_doc_ref(self):
    #     if self.upload_payment_doc and not self.payment_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Payment Confirmation Document.")
    #
    # @api.onchange('residance_doc')
    # def _check_residance_doc_ref(self):
    #     if self.residance_doc and not self.residance_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Residence Permit Document.")
    #
    # @api.onchange('transfer_confirmation_doc')
    # def _check_transfer_confirmation_ref(self):
    #     if self.transfer_confirmation_doc and not self.transfer_confirmation_ref:
    #         raise ValidationError("Ref No.* is required when uploading Confirmation of Transfer.")
    #
    # @api.onchange('muqeem_print_doc')
    # def _check_muqeem_print_doc_ref(self):
    #     if self.muqeem_print_doc and not self.muqeem_print_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Muqeem Print Document.")
    #
    # @api.onchange('upload_upgrade_insurance_doc')
    # def _check_upgrade_insurance_doc_ref(self):
    #     if self.upload_upgrade_insurance_doc and not self.upgarde_ins_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Insurance Upgrade Document.")
    #
    # @api.onchange('upload_iqama_card_doc')
    # def _check_iqama_card_doc_ref(self):
    #     if self.upload_iqama_card_doc and not self.iqama_card_ref:
    #         raise ValidationError("Ref No.* is required when uploading Iqama Card.")
    #
    # @api.onchange('upload_iqama_card_no_doc')
    # def _check_iqama_card_no_doc_ref(self):
    #     if self.upload_iqama_card_no_doc and not self.iqama_card_no_ref:
    #         raise ValidationError("Ref No.* is required when uploading Iqama Card.")
    #
    # @api.onchange('upload_qiwa_doc')
    # def _check_qiwa_doc_ref(self):
    #     if self.upload_qiwa_doc and not self.qiwa_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Qiwa Contract.")
    #
    # @api.onchange('upload_gosi_doc')
    # def _check_gosi_doc_ref(self):
    #     if self.upload_gosi_doc and not self.gosi_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading GOSI Update.")
    #
    # @api.onchange('profession_change_doc')
    # def _check_profession_change_doc_ref(self):
    #     if self.profession_change_doc and not self.profession_change_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Profession Change Req. Document.")
    #
    # @api.onchange('profession_change_final_doc')
    # def _check_profession_change_final_doc_ref(self):
    #     if self.profession_change_final_doc and not self.prof_change_final_ref:
    #         raise ValidationError("Ref No.* is required when uploading Final Profession Change Document.")
    #
    # @api.onchange('upload_salary_certificate_doc')
    # def _check_salary_certificate_doc_ref(self):
    #     if self.upload_salary_certificate_doc and not self.salary_certificate_ref:
    #         raise ValidationError("Ref No.* is required when uploading Salary Certificate.")
    #
    # @api.onchange('upload_bank_letter_doc')
    # def _check_bank_letter_doc_ref(self):
    #     if self.upload_bank_letter_doc and not self.bank_letter_ref:
    #         raise ValidationError("Ref No.* is required when uploading Bank Letter.")
    #
    # @api.onchange('upload_vehicle_lease_doc')
    # def _check_vehicle_lease_doc_ref(self):
    #     if self.upload_vehicle_lease_doc and not self.vehicle_lease_ref:
    #         raise ValidationError("Ref No.* is required when uploading Vehicle Lease Letter.")
    #
    # @api.onchange('upload_apartment_lease_doc')
    # def _check_apartment_lease_doc_ref(self):
    #     if self.upload_apartment_lease_doc and not self.apartment_lease_ref:
    #         raise ValidationError("Ref No.* is required when uploading Apartment Lease Letter.")
    #
    # @api.onchange('upload_family_visa_letter_doc')
    # def _check_family_visa_letter_doc_ref(self):
    #     if self.upload_family_visa_letter_doc and not self.family_visa_letter_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Family Visa Letter.")
    #
    # @api.onchange('upload_visit_visa_app_doc')
    # def _check_visit_visa_app_doc_ref(self):
    #     if self.upload_visit_visa_app_doc and not self.visit_visa_app_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Visit Visa Application.")
    #
    # @api.onchange('upload_family_visit_visa_doc')
    # def _check_family_visit_visa_doc_ref(self):
    #     if self.upload_family_visit_visa_doc and not self.family_visit_visa_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Family Visit Visa Document.")
    #
    # @api.onchange('upload_employment_contract_doc')
    # def _check_employment_contract_doc_ref(self):
    #     if self.upload_employment_contract_doc and not self.employment_contract_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Employment Contract.")
    #
    # @api.onchange('upload_cultural_letter_doc')
    # def _check_cultural_letter_doc_ref(self):
    #     if self.upload_cultural_letter_doc and not self.cultural_letter_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Cultural Letter.")
    #
    # @api.onchange('draft_istiqdam')
    # def _check_istiqdam_doc_ref(self):
    #     if self.draft_istiqdam and not self.istiqdam_form_doc_ref:
    #         raise ValidationError("Ref No.* is required when uploading Istiqdam Form.")

    def download_document(self):
        for record in self:
            if record.document_attachment:
                # Prepare the new filename
                iqama_no = record.iqama_no or 'unknown_iqama'
                employee_name = record.employee_name or 'unknown_employee'
                activity_name = record.service_request_activity or 'unknown_activity'
                document_name = record.document_attachment.name or 'unknown_document'

                # Create the new filename
                new_filename = f"{iqama_no}_{employee_name}_{activity_name}_{document_name}"

                # Update the attachment's filename to the new one
                attachment = record.document_attachment
                attachment.write({
                    'name': new_filename
                })

                # Get the attachment ID and use it to download the file with the updated name
                return {
                ...
                raise ValidationError("No document attached to download.")
    ...
    def action_submit(self):
        """Validate mandatory fields during submission."""
        # Check for reference numbers against uploaded documents
        if self.upload_hr_card and not self.hr_card_ref:
            raise ValidationError("HR Card Reference is required when uploading HR Card Document.")
        if self.upload_jawazat_doc and not self.jawazat_doc_ref:
            raise ValidationError("Jawazat Document Reference is required when uploading Jawazat Document.")
        if self.upload_sponsorship_doc and not self.sponsorship_doc_ref:
            raise ValidationError("Sponsorship Document Reference is required when uploading Sponsorship Document.")
        if self.upload_payment_doc and not self.payment_doc_ref:
            raise ValidationError("Payment Document Reference is required when uploading Payment Document.")
        if self.residance_doc and not self.residance_doc_ref:
            raise ValidationError("Residence Document Reference is required when uploading Residence Document.")

        # Add additional checks for other fields if necessary

        # Proceed with other logic after validation
        # For example, updating the status of the record or performing other actions
    ...
    def open_assign_employee_wizard(self):
        ...
        for line in self:
                ...
                for srt in treasury_id:
                    if srt.state != 'done':
                        raise ValidationError(
                            _('Action required by Finance team. Kindly upload Confirmation Document provided by Treasury Department before continuing further'))
            ...
            return {
                ...
                'view_mode': 'form',
                'target': 'new',
                'context': {'default_department_ids': department_ids, 'default_assign_type': 'assign',
    ...
    def open_reassign_employee_wizard(self):
        ...
        for line in self:
            ...
            return {
                ...
                'view_mode': 'form',
                'target': 'new',
                'context': {'default_department_ids': department_ids, 'default_assign_type': 'reassign',
        ...
        for record in self:
            ...
            if record.aamalcom_pay and record.service_request == 'new_ev' or record.service_request == 'hr_card' or record.service_request == 'iqama_renewal' or record.service_request == 'exit_reentry_issuance' or record.service_request == 'prof_change_qiwa':
                if pricing_id:
                    ...
                    raise ValidationError(
                        _('Service Pricing is not configured properly. Kindly contact your Accounts Manager'))
    ...
    @api.onchange('upload_upgrade_insurance_doc', 'upload_iqama_card_no_doc', 'upload_iqama_card_doc',
                  'upload_qiwa_doc',
                  'upload_gosi_doc', 'upload_hr_card', 'upload_jawazat_doc', 'upload_sponsorship_doc',
                  'upload_confirmation_of_exit_reentry', 'upload_exit_reentry_visa', 'profession_change_doc',
                  'upload_payment_doc', 'profession_change_final_doc', 'upload_salary_certificate_doc',
                  'upload_bank_letter_doc', 'upload_vehicle_lease_doc',
                  'upload_apartment_lease_doc', 'upload_istiqdam_form_doc', 'upload_family_visa_letter_doc',
                  'upload_employment_contract_doc',
                  'upload_cultural_letter_doc', 'upload_family_visit_visa_doc',
                  'upload_emp_secondment_or_cub_contra_ltr_doc', 'upload_car_loan_doc', 'upload_bank_loan_doc',
                  'upload_rental_agreement_doc',
                  'upload_exception_letter_doc', 'upload_attestation_waiver_letter_doc', 'upload_embassy_letter_doc',
                  'upload_istiqdam_letter_doc',
                  ...
                  'upload_final_exit_issuance_doc', 'upload_soa_doc',
                  'upload_sec_doc', 'residance_doc', 'transfer_confirmation_doc', 'muqeem_print_doc',
                  'upload_issuance_doc', 'upload_enjaz_doc', 'e_wakala_doc')
